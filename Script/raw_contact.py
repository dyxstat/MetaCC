...
class ContactMatrix:
    ...
    def __init__(self, bam_file, enzymes, seq_file, path , min_mapq=30, min_len=1000, min_match=30, min_signal=2):
        ...
        logger.info('Writing raw contact matrix...')
        ...
        logger.info('Writing Hi-C signals of contigs...')
        self.seq_map = self.seq_map.tocsr()
    ...
    def _write_contig_info(self):
        with open(os.path.join(self.path , 'contig_info.csv'),'w') as out:
            for seq in self.seq_info:
                out.write(str(seq.name)+ ',' +str(seq.sites)+ ',' +str(seq.length) + ',' +str(seq.covcc))
                out.write('\n')
        
        
    def _write_rowSum(self):
        with open(os.path.join(self.path ,'row_sum.csv'),'w') as out:
...
class ContactMatrix_LC:
    ...
    def __init__(self, bam_file, seq_file, path , min_mapq=30, min_len=1000, min_match=30, min_signal=2):
        ...
        ##test that BAM file is the correct sort order
            ...
            # minimum length threshold
                ...
                self.seq_info.append(SeqInfo_LC(localid , n , rname, rlen, 0)) ######initially set the covcc coverage to be zero
                localid = localid + 1
                offset += rlen
        ...
        for i , idn in enumerate(contig_id):
            seq = self.seq_info[idn]
            assert seq.localid == idn, 'the local index does not match the contact matrix index'
            seq_temp.append(SeqInfo_LC(i , seq.refid , seq.name , seq.length , self.seq_map[idn , idn]))
    ...
    def _write_contig_info(self):
        with open(os.path.join(self.path , 'contig_info.csv'),'w') as out:
            for seq in self.seq_info:
                out.write(str(seq.name)+ ',' +str(seq.length) + ',' +str(seq.covcc))
                out.write('\n')
